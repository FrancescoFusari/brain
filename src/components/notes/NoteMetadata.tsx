  ...
  return (
    <div className="relative rounded-sm bg-muted/50 backdrop-blur-sm border border-border/5">
      ...
      <div className="p-8 md:p-10">
        <h3 className="text-lg md:text-xl font-semibold text-foreground mb-8">
        ...
        <ScrollArea className="h-full max-h-[600px] pr-8">
          <div className="space-y-8">
            {sections.map((section, index) => {
              ...
              return (
                <div key={index}>
                  <h4 className="text-sm font-medium text-foreground/90 mb-4">
                    {section.title}
                  </h4>
                  {section.type === 'badges' ? (
                    <div className="flex flex-wrap gap-2.5">
                      {(section.content as string[]).map((item, badgeIndex) => (
                        <Badge 
                          key={badgeIndex} 
                          variant="secondary"
                          className="px-4 py-1.5 text-sm bg-accent/50 hover:bg-accent/70 transition-colors text-foreground/90"
                        >
                          {item}
                        </Badge>
                      ))}
